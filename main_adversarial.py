...
ALGORITHMS_ONLINE = (
  ...
  algorithms_adversarial.ClassicDet,
  algorithms_adversarial.ClassicRandom,
  algorithms_adversarial.OPT,
  # algorithms.Rent,
)

ALGORITHMS_PRED= (
  algorithms_adversarial.FTP,
  algorithms_adversarial.OLDynamic,
  algorithms_adversarial.OLStatic,
  algorithms_adversarial.Buy,
  # algorithms_adversarial.PACPredictRand1,
...
def RunnerForPool(algorithm, requests, predictions, sigmas):
  return algorithms_adversarial.Cost_history(algorithm(requests, predictions, sigmas), requests, algorithm.__name__)
...
def RunnerForPoolPAC(algorithm, requests, predictions, sigmas):
  ...
  return algorithms_adversarial.Cost_history(history, nature, algorithm.__name__), nature
  # return algorithms_cumulative.CR_history(algorithm(requests, predictions, sigmas), requests, algorithm.__name__)
...
def PlotPredRandom(datasets, preds, num_runs, output_basename=None, load_json=None, keep='rhos', res_type="cr"):
  ...
  predictor = lambda sigma: algorithms_adversarial.PredRandom(requests, sigma)
  ...
  with Pool() as pool:
    grid = list(itertools.product(range(len(ALGORITHMS_PAC)), range(num_runs)))
    args = [(ALGORITHMS_PAC[algorithm_idx], requests, predictions[run_idx], sigmas)
            for algorithm_idx, run_idx in grid]
    runs = pool.starmap(RunnerForPoolPAC, args)
    natures = np.zeros((num_runs,len(requests)))
    for (algorithm_idx, run_idx), (cost, nature) in zip(grid, runs):
      costs_history[algorithm_idx,:,run_idx] = cost
      natures[run_idx,:] = nature

    grid = list(itertools.product(range(len(ALGORITHMS_PRED)), range(num_runs)))
    args = [(ALGORITHMS_PRED[algorithm_idx], natures[run_idx], predictions[run_idx], sigmas)
            for algorithm_idx, run_idx in grid]
    runs = pool.starmap(RunnerForPool, args)
    for (algorithm_idx, run_idx), cost in zip(grid, runs):
      costs_history[len(ALGORITHMS_PAC) + algorithm_idx,:,run_idx] = cost

  for i, algorithm in enumerate(ALGORITHMS_ONLINE):
      for run in range(num_runs):
        output = algorithm(natures[run])
        cost = algorithms_adversarial.Cost_history(output,natures[run], algorithm.__name__)
        # cost = algorithms_cumulative.CR_history(output,requests, algorithm.__name__)
        # CUMULATIVE LOSS
        costs_history[len(ALGORITHMS_PAC)+len(ALGORITHMS_PRED)+i,:,run] = cost
  print(costs_history.shape)

  cumulative_losses = costs_history - costs_history[-1]
