...
def return_adv(dist_full, l, u, max_val):
  assert len(dist_full) == max_val
  lst = [np.sum([(B+i)*dist_full[i] for i in range(N)]) / min(N,B)
        + N * np.sum(dist_full[N:]) / min(N,B)
         for N in range(1,max_val+1)]
  eta = np.argmax(lst[int(l)-1:int(u)]) + l + 1
  gamma = np.argmax(lst) + 1
  return eta, gamma

def PACPredictRand(requests, pred, sigmas, d):
  # NOTE: assumes skiing days lower bounded by 1
  max_val = int(np.ceil(max(requests)))
  nature = [0] * len(requests)
  result = [0] * len(requests)
  z = norm.ppf(1-d/2)
  for t, req in enumerate(requests):
    if t % 1000 == 0:
      print(t)
    l, u = np.floor(max(1,pred[t] - z*sigmas[t])), np.ceil(min(max_val-1,pred[t] + z*sigmas[t]))
    l = min(l,u)
    if u < B:
      c = np.zeros((B+2,))
      c[:2] = np.array([1-d,d])
    ...
    if u < B:
      ...
      dist_full = np.zeros((max_val,))
      dist_full[:B] = dist
      eta, gamma = return_adv(dist_full, l, u, max_val)
    else:
      dist[dist < 0] = 0
      ...
      dist_full = np.zeros((max_val,))
      dist_full[:B] = dist[:B]
      dist_full[int(u)] = dist[B]
      eta, gamma = return_adv(dist_full, l, u, max_val)
    result[t] = action
    nature[t] = np.random.choice([eta, gamma], p=[1-d,d])
...
def OnlineLearning(requests, pred, sigmas, s_num=4, epsilon=0.1, A=1, B=B):
  diameter = 0.5
  beta = 1.0 - 0.5 * epsilon
  beta2 = 0.8
  sigma_num = s_num
  z = norm.ppf(1-0.1/2)
  # l, u = pred[t] - z*sigmas[t], pred[t] + z*sigmas[t]
  L = [max(1,pred[t]-z*sigmas[t]) for t in range(len(sigmas))]
  U = [min(8,pred[t]+z*sigmas[t]) for t in range(len(sigmas))]
